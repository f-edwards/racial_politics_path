...
1. pull immig + urban data
2. pull tenant farmer data
3. model employment -> each, consider loglinear process?

theory re: white immigs: coercion / rehab in NE, yeoman uplift in W, frontier as panacea for urban, lots of hist on that, gets at SocWork/Police vs Teachers employment
...
SETTLER COLONIALISM AS STRUCTURE - PROCESS. BUT SPATIALLY VARIABLE DEPENDING ON LOCAL INTERESTS
...
### Measuring the size, power, and interests of the expropriative settler polity 
...
PULL IMMIG + URBAN - UNIQUE COMBO THAT PUSHES FORWARD WELFARE STATE
...
```{r urban-ts, fig.cap = "Urban white immigrant population by state 1870-1940"}
p1<-ggplot(dat%>% 
             filter(year<=1930) %>% 
             filter(!(is.na(region))),
...
dat %>% 
  filter(year == 1920) %>% 
  mutate(state = tolower(statename)) %>% 
  ggplot(aes(fill = log(white_immig))) + 
  geom_map(aes(map_id = state), map = map) + 
  expand_limits(x = map$long, y = map$lat) + 
  scale_fill_continuous(type = "viridis") + 
  theme_void() + 
...
sc_police<-lmer(log(police_rate) ~ 
              scale(hsclaim_prop) +
              year + 
              (1 + year|state),
            data = model_dat,
